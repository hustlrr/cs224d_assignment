...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ...
            tf.get_variable(name="W1", shape=[2 * self.config.embed_size, self.config.embed_size])
            tf.get_variable(name="b1", shape=[self.config.embed_size])
        with tf.variable_scope('Projection'):
            tf.get_variable(name="U", shape=[self.config.embed_size, self.config.label_size])
            tf.get_variable(name="bs", shape=[self.config.label_size])
    ...
    def add_model(self, node):
        ...
        with tf.variable_scope('Composition', reuse=True):
            ### YOUR CODE HERE
            embedding = tf.get_variable(name="embedding", shape=[len(self.vocab), self.config.embed_size])
            W1 = tf.get_variable(name="W1", shape=[2 * self.config.embed_size, self.config.embed_size])
            b1 = tf.get_variable(name="b1", shape=[self.config.embed_size])
        ...
        if node.isLeaf:
            ...
            curr_node_tensor = tf.expand_dims(tf.gather(embedding, word_idx), dim=0)
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            left_tensor = node_tensors[node.left]
            right_tensor = node_tensors[node.right]
            tmp = tf.concat((left_tensor, right_tensor), axis=1)
            curr_node_tensor = tf.nn.xw_plus_b(x=tmp, weights=W1, biases=b1)
    ...
    def add_projections(self, node_tensors):
        ...
        with tf.variable_scope('Projection', reuse=True):
            U = tf.get_variable(name="U", shape=[self.config.embed_size, self.config.label_size])
            bs = tf.get_variable(name="bs", shape=[self.config.label_size])
            logits = tf.nn.xw_plus_b(x=node_tensors, weights=U, biases=bs)
            l2_loss = tf.get_collection("l2_loss")[0]
            l2_loss += tf.nn.l2_loss(U)
            l2_loss += tf.nn.l2_loss(bs)
            tf.add_to_collection("l2_loss", l2_loss)
        ### END YOUR CODE
        return logits
    ...
    def predictions(self, y):
        ...
        predictions = None
        # YOUR CODE HERE
        predictions = tf.arg_max(y, dimension=1)
        # END YOUR CODE
        return predictions
