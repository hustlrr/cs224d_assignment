...
class NERModel(LanguageModel):
    ...
    def add_model(self, window):
        ...
        with tf.name_scope("Layer"):
            ...
            b1 = tf.get_variable(name="b1", shape=[self.config.hidden_size], dtype=tf.float32,
                                 initializer=tf.contrib.layers.xavier_initializer())
            l2_loss += tf.nn.l2_loss(W)
            l2_loss += tf.nn.l2_loss(b1)
            self.h = tf.nn.relu(tf.nn.xw_plus_b(x=window, weights=W, biases=b1), name="relu")
        ...
        with tf.name_scope("Softmax"):
            ...
            output = tf.nn.xw_plus_b(x=self.h_dropout, weights=U, biases=b2)
        ### END YOUR CODE
        return output
    ...
    def add_loss_op(self, y):
        ...
        """
        ### YOUR CODE HERE
        losses = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=y)
...
def test_NER():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            _, predictions = model.predict(session, model.X_test, y=None)
            save_predictions(predictions, "data/ner/q2_test.predicted")
