...
class SoftmaxModel(Model):
    ...
    def add_model(self, input_data):
        ...
        """
        ### YOUR CODE HERE
        with tf.name_scope("linear_layer_softmax"):
            W = tf.Variable(tf.constant(0.0, shape=[self.config.n_features, self.config.n_classes]),
                            trainable=True, name="W")
            b = tf.Variable(tf.constant(0.0, shape=[self.config.n_classes]),
                            trainable=True, name="b")
            out = tf.nn.xw_plus_b(x=input_data, weights=W, biases=b, name="y")
        ### END YOUR CODE
        return out
