...
def softmax(x):
    ...
    with tf.device("/cpu:0"):
        x_row_max = tf.reduce_max(x, axis=1, keep_dims=True)
        x = tf.exp(x - x_row_max)
        x_row_sum = tf.reduce_sum(x, axis=1, keep_dims=True)
        out = tf.divide(x, x_row_sum)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    with tf.device("/cpu:0"):
        y = tf.to_float(y)
        yhat = tf.to_float(tf.log(yhat))
        out = -tf.reduce_sum(tf.multiply(y, yhat))
...
def test_cross_entropy_loss_basic():
    ...
    session_conf = tf.ConfigProto(
        allow_soft_placement=True)
    with tf.Session(config=session_conf), tf.device("/cpu:0"):
        test1 = test1.eval()
    result = -3 * np.log(.5)
